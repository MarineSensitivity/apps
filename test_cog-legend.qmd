...
b1 <- c(xmin = x, xmax = x+1, ymax = y+1, ymin = y) |>
  st_bbox(crs = st_crs(4326)) |> 
  st_as_sfc() |> 
  st_as_sf() |> 
  mutate(
    val = cog_rng[1])
b2 <- c(xmin = x-1, xmax = x, ymax = y, ymin = y-1) |>
  st_bbox(crs = st_crs(4326)) |> 
  st_as_sfc() |> 
  st_as_sf() |> 
  mutate(
...
rdeck(
  ...
  add_polygon_layer(
    ...
    get_fill_color = scale_color_linear(
      ...
      palette = scales::colour_ramp(RColorBrewer::brewer.pal(11, "Spectral")),
      val, limits = cog_rng)) |> 
  add_tile_layer(
    id      = "base",
    opacity = 1,
